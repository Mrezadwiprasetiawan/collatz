...
static void pushVert(std::vector<float>& buf,
    ...
    buf.insert(buf.end(), {p.x, p.y, p.z, r, g, b, a});
}
...
class ArrowPlot {
public:
    struct Point { float x, y, z; std::string label; };

    // ── add points ───────────────────────────────────────────────

    void add(float x, float y, float z, const std::string& lbl = "") {
        pts_.push_back({x, y, z,
            lbl.empty() ? "P"+std::to_string(pts_.size()+1) : lbl});
    }
    ...
    Mesh build(const Config& cfg = {}, const ColourScheme& col = {}) const {
        if (pts_.size() < 2)
            throw std::invalid_argument("ArrowPlot: need at least 2 points");
...
private:
    ...
    void buildSphere(std::vector<float>& buf, Vec3 c,
        ...
        auto pt = [&](int stack, int slice) -> Vec3 {
            ...
            return {
                c.x + r * std::sin(phi) * std::cos(theta),
                c.y + r * std::cos(phi),
                c.z + r * std::sin(phi) * std::sin(theta)
            };
        };
    ...
    void buildCylinder(std::vector<float>& buf,
                       Vec3 A, Vec3 B, float radius, int slices,
                       float R, float G, float Bl, float Al) const {
        ...
        auto ring = [&](Vec3 centre, int sl) -> Vec3 {
            ...
            return centre + u * (radius * std::cos(theta))
    ...
    void buildCone(std::vector<float>& buf,
                   Vec3 tip, Vec3 baseCenter, float baseRadius, int slices,
                   float R, float G, float Bl, float Al) const {
        ...
        auto baseRing = [&](int sl) -> Vec3 {
            float theta = 2*kPi * (float)sl / slices;
            return baseCenter + u * (baseRadius * std::cos(theta))
                              + v * (baseRadius * std::sin(theta));
        };
    ...
    void buildArrow(Mesh& mesh, Vec3 A, Vec3 B,
                    const Config& cfg, const ColourScheme& col) const {
        ...
        buildCylinder(mesh.shaftVerts,
                      shaftStart, shaftEnd,
                      cfg.shaftRadius, cfg.shaftSlices,
                      col.shaftR, col.shaftG, col.shaftB, col.shaftA);

        buildCone(mesh.headVerts,
                  tip, coneBase,
                  cfg.headRadius, cfg.headSlices,
                  col.headR, col.headG, col.headB, col.headA);
    }
};
